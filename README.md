...
これはデスクトップコンポジター (DWM) が実際に画面へ出しているフレームレートで、余裕が
あるうちはリフレッシュレートに張り付き、GPU が追いつかなくなると落ちます。特定のゲーム
やアプリ内部のフレームレートではありません。

> 実装メモ: `DwmGetCompositionTimingInfo` は現行の Windows 11 では hwnd を問わず
> `0x88980090` で失敗するため、次のフレームまでブロックする `DwmFlush` を数回呼んで
> その所要時間から実測しています。
